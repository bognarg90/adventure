...
import com.mycompany.adventure.Levels.InventoryOverlay;
import com.mycompany.adventure.Levels.World;
import com.mycompany.adventure.TileInteraction.Objects.Item;
import com.mycompany.adventure.TileInteraction.Objects.ItemDurabilityBar;
        ...
        for(int y = 0; y < slots.size(); y++)
        {
            if(slots.indexOf(slots.get(y)) == slotSelected - 1)
            {
                ...
                try
                {
                    ...
                    if(!renderOverlay) nameDrawer.draw
                    (slotBatch, inventory.get(y).stackedItem.name, (inventory.get(y).stackedItem.sprite.getX() +
